...
/**
 * Premium sahifasi — bosh sahifa va blog bilan bir xil V2 qobig'ida
 ...
 */
export default async function PremiumLayout({ children, params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: "v2" });
  const navBotUrl = botDeepLink({ page: "premium", placement: "nav" });
  const footerBotUrl = botDeepLink({ page: "premium", placement: "footer" });
